BEGIN;
-- 044: Trial provision — elevators as jsonb with name + floors_count (public.elevators.floors_count).
-- Replaces provision_sales_lead_trial_portal signature (drops text[] overload).
...
begin
  ...
  if p_elevators is null or jsonb_typeof(p_elevators) <> 'array' then
    raise exception 'missing_elevators';
  end if;

  v_elevator_len := jsonb_array_length(p_elevators);
  if v_elevator_len is null or v_elevator_len < 1 then
    raise exception 'missing_elevators';
  end if;
  ...
  if v_building_service_type is distinct from 'אחר' then
    v_building_service_type_other := null;
  end if;
  ...
  for v_i in 0 .. v_elevator_len - 1 loop
    v_elevator := p_elevators -> v_i;

    if v_elevator is null or jsonb_typeof(v_elevator) <> 'object' then
      raise exception 'invalid_elevator_name';
    end if;

    v_elevator_name := nullif(trim(coalesce(v_elevator ->> 'name', '')), '');
    if v_elevator_name is null then
      raise exception 'invalid_elevator_name';
    end if;

    if not (v_elevator ? 'floors_count') then
      raise exception 'invalid_floors_count';
    end if;

    if jsonb_typeof(v_elevator -> 'floors_count') <> 'number' then
      raise exception 'invalid_floors_count';
    end if;

    if (v_elevator -> 'floors_count')::text ~ '\.' then
      raise exception 'invalid_floors_count';
    end if;

    begin
      v_floors_count := (v_elevator ->> 'floors_count')::integer;
    exception
      when others then
        raise exception 'invalid_floors_count';
    end;

    if v_floors_count is null or v_floors_count <= 0 then
      raise exception 'invalid_floors_count';
    end if;

    v_elevator_id := 'e' || (v_i + 1)::text;
...
revoke all on function public.provision_sales_lead_trial_portal(uuid, timestamptz, jsonb) from public, anon, authenticated;
grant execute on function public.provision_sales_lead_trial_portal(uuid, timestamptz, jsonb) to service_role;
