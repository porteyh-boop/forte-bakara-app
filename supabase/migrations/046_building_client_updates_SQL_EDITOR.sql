-- 046 SQL Editor — building client updates + can_view_client_updates
-- Safe to re-run: CREATE IF NOT EXISTS, ADD COLUMN IF NOT EXISTS, no data deletes.
-- Requires: 005 client_access, 008 documents, 013 client_permissions
-- Run entire script once in Supabase SQL Editor (Production).
...
-- ---------------------------------------------------------------------------
-- Client permission (existing rows keep false via DEFAULT)
-- ---------------------------------------------------------------------------
