...
-- 042: Fix trial portal token generation (pgcrypto schema qualification).
-- Does NOT re-run 041. Replaces provision_sales_lead_trial_portal only.
--
-- Read-only catalog (run before apply if unsure):
--   select extname, n.nspname from pg_extension e join pg_namespace n on n.oid = e.extnamespace where extname = 'pgcrypto';
--   select n.nspname, p.proname from pg_proc p join pg_namespace n on n.oid = p.pronamespace where p.proname = 'gen_random_bytes';
--   select coalesce(array_to_string(p.proconfig, ', '), '') from pg_proc p join pg_namespace n on n.oid = p.pronamespace where n.nspname = 'public' and p.proname = 'provision_sales_lead_trial_portal';
--
-- 041 sets search_path = public; on Supabase, gen_random_bytes lives in extensions after pgcrypto is enabled.
