-- Run once in Supabase SQL Editor (single transaction).
-- Source: supabase/migrations/041_sales_lead_trial_portal.sql

BEGIN;

-- Sales lead trial committee portal (additive).
-- service_role only for RPCs. Do not run in production until approved.

alter table public.buildings
  add column if not exists is_trial boolean not null default false;

create index if not exists idx_buildings_is_trial
  on public.buildings (is_trial)
  where is_trial = true;

alter table public.sales_leads
  add column if not exists trial_building_id text;

alter table public.sales_leads
  add column if not exists trial_client_user_id uuid references public.client_users (id) on delete set null;

create unique index if not exists idx_sales_leads_trial_building_id_unique
  on public.sales_leads (trial_building_id)
  where trial_building_id is not null;

create index if not exists idx_sales_leads_trial_client_user_id
  on public.sales_leads (trial_client_user_id);
...
begin
  if not exists (
    select 1
      from pg_constraint
     where conname = 'sales_leads_trial_building_id_fkey'
       and conrelid = 'public.sales_leads'::regclass
  ) then
    alter table public.sales_leads
      add constraint sales_leads_trial_building_id_fkey
      foreign key (trial_building_id)
      references public.buildings (building_id)
      on delete set null;
  end if;
end $$;

comment on column public.buildings.is_trial is
  'Trial committee portal building ג€” excluded from active client / business metrics until win conversion.';
comment on column public.sales_leads.trial_building_id is
  'buildings.building_id for pre-win trial portal. Idempotent provision reuses this link.';
comment on column public.sales_leads.trial_client_user_id is
  'Primary client_users row for the trial portal link on this lead.';

-- Atomic trial provision: one building + elevators + portal link per lead.
...
begin
  ...
  insert into public.buildings (
    ...
    'standard',
    null,
    nullif(trim(coalesce(v_lead.service_type, '')), ''),
    nullif(trim(coalesce(v_lead.service_type_other, '')), '')
  );
...
-- Win conversion: reuse trial building when present; clear is_trial; do not touch client_users.expires_at.
...
begin
  ...
  if nullif(trim(coalesce(v_lead.trial_building_id, '')), '') is not null then
    v_building_id := trim(v_lead.trial_building_id);

    update public.buildings
       set name = trim(p_name),
           city = nullif(trim(coalesce(p_city, '')), ''),
           address = nullif(trim(coalesce(p_address, '')), ''),
           management_company = nullif(trim(coalesce(p_management_company, '')), ''),
           contact_name = nullif(trim(coalesce(p_contact_name, '')), ''),
           contact_phone = nullif(trim(coalesce(p_contact_phone, '')), ''),
           project_notes = nullif(trim(coalesce(p_project_notes, '')), ''),
           project_type = coalesce(nullif(trim(coalesce(p_project_type, '')), ''), 'standard'),
           order_amount = p_order_amount,
           service_type = v_service_type,
           service_type_other = v_service_type_other,
           is_trial = false,
           project_stage = '׳”׳–׳׳ ׳”'
    ...
    if v_updated_rows = 0 then
      raise exception 'trial_building_missing';
    end if;
