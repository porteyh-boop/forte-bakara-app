...
import { listClientBuildingClientUpdatesServer } from "@/lib/building-client-updates-server";
...
export async function GET(request: NextRequest) {
  ...
  const result = await listClientBuildingClientUpdatesServer(authResult.auth);
  ...
  return clientPortalJson({ updates: result.updates });
}
